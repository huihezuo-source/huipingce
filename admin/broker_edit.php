...
<form method="post" action="/admin/api/broker_save.php" class="form-grid" style="grid-template-columns:1.4fr 1fr;align-items:start">
  ...
  <div>
    <div class="card">
      ...
      <div class="form-grid-3">
        <div class="form-row"><label>注册地/国家</label><input class="inp" name="country" value="<?= $v('country') ?>"></div>
        <div class="form-row"><label>总部</label><input class="inp" name="headquarters" value="<?= $v('headquarters') ?>"></div>
        <div class="form-row"><label>展业范围</label><input class="inp" name="scope" value="<?= $v('scope') ?>" placeholder="全球展业/区域性"></div>
      </div>
      <div class="form-row"><label>一句话简介</label><textarea class="inp" name="summary" rows="2"><?= $v('summary') ?></textarea></div>
      <div class="form-row"><label>标签（逗号分隔）</label><input class="inp" name="tags" value="<?= $v('tags') ?>" placeholder="ECN,低点差,MT4"></div>
      <div class="form-row"><label>公司介绍（详情页长文，支持 HTML）</label><textarea class="inp" name="intro" rows="5"><?= $b?h($b['intro']):'' ?></textarea></div>
      <div class="form-grid-3">
      ...
      <div class="form-grid-3">
        ...
        <div class="form-row"><label>官方评分（暂缓）</label><input class="inp" type="number" step="0.1" name="score" value="<?= $b&&$b['score']!==null?$v('score'):'' ?>" placeholder="本期留空，用户评分自动聚合"></div>
      </div>
