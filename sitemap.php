...
u($base.'/brokers.php', time(), 'daily', '0.8');
u($base.'/reviews.php', time(), 'daily', '0.9');
...
foreach (db()->query('SELECT id,updated_at FROM brokers')->fetchAll() as $b)
  u($base.'/broker-detail.php?id='.$b['id'], $b['updated_at'] ?: time(), 'weekly', '0.7');

// 评测
foreach (db()->query("SELECT id,COALESCE(publish_at,created_at) AS t FROM reviews WHERE ".published_where())->fetchAll() as $rv)
  u($base.'/review-detail.php?id='.$rv['id'], $rv['t'], 'monthly', '0.8');
